                  ...
                  int ip, int port, int numTransac)
{
	...
	while (LireInformations(f, &infos)) {
		if (infos.Action == RESERVATION
			...
			// Enregistre la demande de paiement
			...
			return true;
		} else if (infos.Action == VALIDERPAYEMENT
		           && infos.NumTicket == numTicket) {
...
// Retourne -1 en cas de ticket non existant
// -2 si déjà sortit
// -3 si le ticket n'est pas encore payé
// -4 si la sortie ne s'effectue pas dans la même heure
// 0 si sortie effectuée
int SortirParking(FILE* f, int numTicket, int heureDepart,
                  int ip, int port, int numTransac)
{
	INFOS infos, entetes;
...
// 	scanf("%d", &numTicket);
	...
	fseek(f, -sizeof infos, SEEK_END);
	while (LireInformations(f, &infos)) {
		if (infos.Action == VALIDERPAYEMENT
		    && infos.NumTicket == numTicket) {
...
// 			scanf("%d", &heureDepart);
		...
		} else if (infos.Action == RESERVATION
		           && infos.NumTicket == numTicket) {
			printf("Ticket non payé\n");
