...
// Transmet une requête au serveur et la renvoie s'il n'y a pas de réponse
...
void Transmettre(struct Physique* conn, int connDescPhys, FILE* f, const char* nomAction, bool loop, bool annuler,
		 ...
		 void (*fct_reception)(FILE* f, struct Physique* conn, int connDescPhys, REQUETE requeteRecue))
{
