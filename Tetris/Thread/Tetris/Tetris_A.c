...
void* fctThreadForme(void* param)
{
   ...
   while(1)
      {
         printf("FORME attend lock\n");
         pthread_mutex_lock(&mutexEvent);
         printf("FORME apres lock\n");
...
void enoughPlace(S_FORME* forme)
{
   int i;
      for (i = 0; i < 4; i++)
         {
	   printf("enough : L=%d C=%d tab=%d\n",forme->L[i],forme->C[i],tab[(forme->L[i])][forme->C[i]]); 
	   fflush(stdout);
