...
void fctThreadGenFormeFin(void *p);
...
{
   param = NULL;
pthread_cleanup_push(fctThreadGenFormeFin, 0);
   ...
   if(sig == SIGUSR1)
      {
		...
		if(ligne->compteur > cptMax)
		{
			cptMax = ligne->compteur;
			maxL = ligne->numLigne;
		}
		if(ligne->compteur < cptMin)
		{
			cptMin = ligne->compteur;
			minL = ligne->numLigne;
		}
