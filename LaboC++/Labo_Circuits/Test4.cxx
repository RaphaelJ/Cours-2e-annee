...
void Essai4()
{
  ...
  for (int i=0 ; i<10 ; i++)   // Tracez vos constructeurs !!!
  {
    ...
    switch(n)
    {
      case 0 : cout << "Competitor" << endl;
		person[i] = new Competitor();
               break;

      case 1 : cout << "FederationAdmin" << endl;
	      person[i] = new FederationAdmin();
               break;

      case 2 : cout << "TeamMember" << endl;
		person[i] = new TeamMember();
               break;
    }
  }
  cout << endl;
  ...
  {
    cout << "Person[" << i << "] ";
    Competitor* pC = dynamic_cast<Competitor*>(person[i]);
    if (pC != NULL) cout << "est un Competitor" << endl;
    
    FederationAdmin*  pF = dynamic_cast<FederationAdmin*>(person[i]);
    if (pF != NULL) cout << "est un FederationAdmin" << endl;
    
    TeamMember* pT = dynamic_cast<TeamMember*>(person[i]);
    if (pT != NULL) cout << "est un TeamMember" << endl;
  }
  cout << endl;
}
