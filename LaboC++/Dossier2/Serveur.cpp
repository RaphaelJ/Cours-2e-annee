...
{	
	...
	if ((IdSem = semget((key_t) 1234, 1, MAX_PUB_SIZE, IPC_CREAT | IPC_EXCL | 0600))
		== -1) {
		TraceErreur("Erreur sémaphore (READQ)");
		exit(1);
	}
	...
	arg.val = 0;
	if (semctl(IdSem, 0, SETVAL, arg)) {
		TraceErreur("Erreur d'initialisation sémaphore (READQ)");
		exit(1);
	}
	
	Trace("Sémaphore ouverte et initialisee");
...
void GererReservation(pid_t borne, int choix)
{
	...
	op.sem_flg = 0;
	semop(IdSem, &op, 1);
	Trace("Semaphore augmentee de 1");
