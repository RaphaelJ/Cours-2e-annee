...
int Ouverture(FICHIER* F,const char* NomFichier)
{
...
int Fermeture(FICHIER F)
{
Trace("Fermeture du fichier");
return close(F.hdF);
}

int Ajout(FICHIER F,const char* Cle,const char * Valeur)
{
...
while (pElm1->OffSet < rc)
   {
     ...
     if (!strcmp(pElm2->Donnee,Cle)) return LibereM(p,0);
...
int Suppression(FICHIER F,const char* Cle)
{
return 0;
}

int OffSetElm(int Taille)
{
int             rc,OffSet;
OffSet = Taille + 7;    // 6 + (Taille + 1)
rc = OffSet %4;
if (rc) OffSet = OffSet + 4 - rc;

return OffSet;
}

void RechercheCle(char* Donnee)
{
int i = 0;
while (Donnee[i] != '=') i++;
Donnee[i] = '\0';
